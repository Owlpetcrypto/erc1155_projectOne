...
const MainMint = ({ accounts, setAccounts }) => {
    ...
    async function handleMint() {
        if (window.ethereum) {
            ...
            //HOW TO SOLVE?????????????????????????????????????????????????
            try {
                const response = await contract.mintOne( {
    ...
    return (
        <Flex justify='center' align='center' height="50vh" paddingBottom='150px' >
            <Box width='1000px'>
                <div>
                    <Image src="https://gateway.pinata.cloud/ipfs/QmV61wWswuVsome2PQbfNPHZ912aBi7vUjn7wTg8yyaMQC/1.png" width="35%" marginTop="500px" borderRadius="10%">
